...
import visible.org.springframework.boot.AutumnApplication.AbandonedRunException;
...
/**
 * Entry point for AOT processing of a {@link AutumnApplication}.
 * <p>
...
public class SpringApplicationAotProcessor extends ContextAotProcessor {
	...
	private static final class AotProcessorHook implements SpringApplicationHook {
		...
		@Override
		public SpringApplicationRunListener getRunListener(AutumnApplication application) {
		...
		private <T> GenericApplicationContext run(ThrowingSupplier<T> action) {
			try {
				AutumnApplication.withHook(this, action);
			}
